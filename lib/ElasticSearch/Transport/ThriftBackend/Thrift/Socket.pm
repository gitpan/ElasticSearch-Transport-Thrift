...
sub open
{
    ...
    my $sock = IO::Socket::INET->new(PeerAddr => $self->{host},
        ...
        || do {
            ...
            die new ElasticSearch::Transport::ThriftBackend::Thrift::TException($error);
...
sub readAll
{
    ...
    while (1) {
        ...
        if(@sockets == 0){
            die new ElasticSearch::Transport::ThriftBackend::Thrift::TException('TSocket: timed out reading '.$len.' bytes from '.
                                       $self->{host}.':'.$self->{port});
        }
        ...
        if (!defined $buf || $buf eq '') {
            ...
            die new ElasticSearch::Transport::ThriftBackend::Thrift::TException('TSocket: Could not read '.$len.' bytes from '.
                               $self->{host}.':'.$self->{port});
...
sub write
{
    ...
    while (length($buf) > 0) {
        ...
        if(@sockets == 0){
            die new ElasticSearch::Transport::ThriftBackend::Thrift::TException('TSocket: timed out writing to bytes from '.
                                       $self->{host}.':'.$self->{port});
        }
        ...
        if (!defined $got || $got == 0 ) {
            die new ElasticSearch::Transport::ThriftBackend::Thrift::TException('TSocket: Could not write '.length($buf).' bytes '.
                                 $self->{host}.':'.$self->{host});
        }
...
sub accept
{
    ...
    if ( exists $self->{handle} and defined $self->{handle} )
    {
        ...
        my $result        = new ElasticSearch::Transport::ThriftBackend::Thrift::Socket;
...
ElasticSearch::Transport::ThriftBackend::Thrift::Socket
