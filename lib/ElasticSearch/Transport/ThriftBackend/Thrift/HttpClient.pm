...
{
  $ElasticSearch::Transport::ThriftBackend::Thrift::HttpClient::VERSION = '0.03';
}
...
sub readAll
{
    ...
    if (!defined($buf)) {
      die new ElasticSearch::Transport::ThriftBackend::Thrift::TException('TSocket: Could not read '.$len.' bytes from input buffer');
    }
    return $buf;
}
...
sub read
{
    ...
    if (!defined($in)) {
      die new ElasticSearch::Transport::ThriftBackend::Thrift::TException("Response buffer is empty, no request.");
    }
    eval {
      my $ret = sysread($in, $buf, $len);
      if (! defined($ret)) {
        die new ElasticSearch::Transport::ThriftBackend::Thrift::TException("No more data available.");
      }
    }; if($@){
      die new ElasticSearch::Transport::ThriftBackend::Thrift::TException($@);
    }
...
ElasticSearch::Transport::ThriftBackend::Thrift::HttpClient
